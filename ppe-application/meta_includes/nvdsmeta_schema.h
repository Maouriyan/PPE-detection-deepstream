...
typedef struct NvDsFaceObjectWithExt {
  ...
  //gint mask_class_id;     /**< Class ID of mask:0 no-mask:1. */
...
typedef struct NvDsEventMsgMeta {
  ...
  gchar *ts;
  // Holds a pointer to the source video path
  gchar *sourceUri;
  // Holds a pointer to a list of pointer of type NvDsObjectMinimalMeta
  GList *obj_min_meta_list;
  GList *line_name_list;
  GList *line_count_list;
  
} NvDsEventMsgMeta;

typedef struct NvDsObjectMinimalMeta {
  gfloat top;     /**< Holds the position of rectangle's top in pixels. */
  gfloat left;    /**< Holds the position of rectangle's left side in pixels. */
  gfloat width;   /**< Holds the rectangle's width in pixels. */
  gfloat height;  /**< Holds the rectangle's height in pixels. */
  
  gint class_id; /** Holds the index of the object class inferred by the primary detector/classifier. */

  guint64 object_id; /** Holds a unique ID for tracking the object. @ref UNTRACKED_OBJECT_ID indicates that the object has not been tracked. */
  guint64 parent_object_id;
  ...
  //gbool mask;
  gchar *obj_label; /** Holds a pointer to astring describing the class of the detected object. */
  gchar *obj_dir;
